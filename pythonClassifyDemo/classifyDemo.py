...
def PostProcess(resultList, dstFilePath, fileName):
	...
	global sendata
	sendata = pickle.dumps(img, protocol=0)
	nLen = len(sendata)
	print 'send data length is:%d' % nLen
	psc.send_data(bytes(str(nLen)))
	event.wait()
	event.clear()
        return None
    else:
...
if __name__ == "__main__":
	def recvdata(data):
		if str(data) == "send":
			...
			psc.send_data(sendata)
		else:
			event.set()
	psc = client.PresenterSocketClient(("192.168.1.246", 5200),5,recvdata)
